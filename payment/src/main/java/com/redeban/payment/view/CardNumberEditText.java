 ...
 */
public class CardNumberEditText extends RedebanEditText {
    ...
    public String getCardNumber() {
        return mIsCardNumberValid
                ? RedebanTextUtils.removeSpacesAndHyphens(getText().toString())
                : null;
    }
    ...
    private void listenForTextChanges() {
        addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ...
                String spacelessNumber = RedebanTextUtils.removeSpacesAndHyphens(s.toString());
    ...
    private void updateCardBrandFromNumber(String partialNumber) {
        String spacelessCardNumber = RedebanTextUtils.removeSpacesAndHyphens(partialNumber);



        if(partialNumber.length() >= 6 && partialNumber.length() <= 10){
            RedebanService redebanService = Payment.getRedebanService(mContext);
            redebanService.cardBin(spacelessCardNumber).enqueue(new Callback<CardBinResponse>() {
                @Override
